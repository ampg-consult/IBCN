...
// Use system ffmpeg installed via apt-get in Dockerfile
...
async function processLaunchpad(jobId, prompt, userId) {
    try {
        ...
        const idea = await retryOperation(() => generateLaunchpadStep(prompt, "Refine into 1-sentence value prop: "), 3);
        await updateJobStatus(jobId, { stage: 'validation', progress: 25 });
        const validation = await retryOperation(() => generateLaunchpadStep(idea, "Provide market validation for: "), 3);
        await updateJobStatus(jobId, { stage: 'plan', progress: 40 });
        const plan = await retryOperation(() => generateLaunchpadStep(idea, "Generate 3-phase execution plan for: "), 3);
        await updateJobStatus(jobId, { stage: 'build', progress: 60 });
        const buildSpecs = await retryOperation(() => generateLaunchpadStep(idea, "List tech stack for: "), 3);
        ...
        await updateJobStatus(jobId, { status: 'completed', stage: 'done', progress: 100, result: finalResult });
    } catch (error) {
...
async function processVideo(jobId, prompt, userId) {
    ...
    try {
        ...
        for (let i = 0; i < scriptData.scenes.length; i++) {
            ...
            await retryOperation(() => generateImage(scriptData.scenes[i].visual, imgPath), 3);
